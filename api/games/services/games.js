...
async function setImage({image, game, field = "cover"}){
  try {
    ...
    formData.append("ref", "games")
    formData.append("field", field)
    formData.append("files", buffer, {filename: `${game.slug}.jpg`})

    console.log(`uploading ${field} image: ${game.slug}.jpg`);
    console.log(`http://${strapi.config.host}:${strapi.config.port}/upload`)
    await axios({
      method: "POST",
  ...
  try {
    await Promise.all(
      products.map(async (product) => {
        const item = await getByName(product.title, "games")
        if(!item){
          console.log(`creating ${product.title}...`)

          const game = await strapi.services.games.create({
            name: product.title,
